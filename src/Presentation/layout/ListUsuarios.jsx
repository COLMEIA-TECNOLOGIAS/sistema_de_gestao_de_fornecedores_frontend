...
const formatDate = (value) => {
  ...
  return isNaN(date.getTime()) ? String(value) : date.toLocaleDateString("pt-AO");
};
...
export default function UsuariosManagementPage() {
  ...
  const fetchUsuarios = useCallback(async () => {
    try {
      setLoading(true);
      // usersAPI.getAll percorre todas as páginas da API (a rota /users é paginada)
      const data = await usersAPI.getAll();
      const usuariosFormatados = (Array.isArray(data) ? data : data?.data || []).map((user, index) => {
        const isActive = user.is_active !== undefined ? user.is_active !== false : (user.status === "Activo" || !!user.ativo);
        return {
        id: user.id || index + 1,
        ...
        data: formatDate(user.dataCriacao || user.createdAt || user.created_at)
        };
      });
      setUsuarios(usuariosFormatados);
      setError(null);
    } catch (err) {
      setError(err.response?.data?.message || err.message || "Erro ao carregar utilizadores");
  ...
  const filteredUsuarios = normalizedQuery
    ? usuarios.filter(u =>
        [u.nome, u.email, u.roleName].some(v => (v || "").toLowerCase().includes(normalizedQuery))
      )
    : usuarios;
        ...
        <div className="bg-white rounded-2xl mb-6 overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <div className="p-8">
              ...
              <p className="text-gray-600">Faça a gestão dos utilizadores registados</p>
            </div>
                ...
                <div className="flex items-center gap-1">
                  {getPageNumbers().map(pageNum => (
                    <button
                      ...
                      className={`w-10 h-10 flex items-center justify-center rounded-lg text-sm font-medium transition-colors ${pageNum === safePage ? 'bg-[#44B16F] text-white' : 'hover:bg-gray-100 text-gray-700'}`}
                    >
                      {pageNum}
                    </button>
                  ))}
