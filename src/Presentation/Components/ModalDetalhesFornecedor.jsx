...
export default function ModalDetalhesFornecedor({
    ...
    const handleViewDocument = async (documentType, title) => {
        ...
        try {
            // Extract type and index if present (e.g. "commercial_license&index=0")
            const [type, query] = documentType.split('&');
            const params = {};
            if (query && query.startsWith('index=')) {
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl mx-4 animate-fadeIn max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="px-8 py-6 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-3">
                    ...
                    {onEdit && (
                        <button
                            onClick={onEdit}
                            className="px-6 py-2.5 bg-[#44B16F] text-white rounded-lg hover:bg-[#3a965d] transition-colors font-medium shadow-sm flex items-center gap-2"
                        >
